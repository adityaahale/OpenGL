...
@private
	...
	GLuint isPerVertexLightingUniformAta, isPerFragmentLightingUniformAta;
...
-(void)prepareOpenGL
{
	...
	isPerVertexLightingUniformAta = glGetUniformLocation(shaderProgramObject_ata, "u_per_vertex"); 
	isPerFragmentLightingUniformAta = glGetUniformLocation(shaderProgramObject_ata, "u_per_fragment");	
...
-(void)drawView
{
	...
	if (lightAta == true) {
		...
		if (isPerVertex) {
			glUniform1i(isPerVertexLightingUniformAta, 1);
			glUniform1i(isPerFragmentLightingUniformAta, 0);
		}
		else if(isPerFragment)
		{
			glUniform1i(isPerFragmentLightingUniformAta, 1);
			glUniform1i(isPerVertexLightingUniformAta, 0);
		}
