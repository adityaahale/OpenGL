...
bool gbIsVertex = true; 
bool gbIsFragment = false;
...
GLuint isVertexLightingUniform, isFragmentLightingUniform;
...
void initialize(void)
{
	...
	const GLchar *vertexShaderSourceCode =
		...
		"uniform int u_is_per_vertex_enable;" \
		"uniform vec3 u_la;" \
		"uniform vec3 u_ld;" \
		"uniform vec3 u_ls;" \
		"uniform vec3 u_ka;" \
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"uniform vec4 u_light_position;" \
		"uniform int u_lkeypress;" \
		"uniform int u_is_per_fragment_enable;" \
		"out vec4 FragColor;" \
		"void main(void)" \
		"{" \
		"vec3 phong_ads_light_local;"
		"if (u_lkeypress == 1)" \
		"{" \
		"if (u_is_per_fragment_enable == 1)" \
		"{" \
		"vec3 norm_tnorm = normalize(tnorm);" \
		"vec3 norm_light_direction = normalize(light_direction);" \
		"float tn_dot_lightd = max(dot(norm_light_direction,norm_tnorm),0.0);" \
		...
		"vec3 diffuse = u_ld * u_kd * tn_dot_lightd;" \
		"vec3 specular = u_ls * u_ks * pow(max(dot(reflection_vector,norm_viewer_vector),0.0),u_material_shin);" \
		"phong_ads_light_local = ambient + diffuse + specular;" \
		"FragColor = vec4(phong_ads_light_local, 1.0);" \
		"}" \
		"else" \
		"{" \
		...
		"{" \
		/*"phong_ads_light = vec3(1.0,1.0,1.0);" \
		"phong_ads_light_local = vec3(1.0, 1.0, 1.0);" \*/
	...
	isVertexLightingUniform = glGetUniformLocation(ShaderProgramObject, "u_is_per_vertex_enable");
	isFragmentLightingUniform = glGetUniformLocation(ShaderProgramObject, "u_is_per_fragment_enable");
...
void display(void)
{
	...
	if (gbLighting == true)
	{
		...
		if (gbIsVertex) {
			glUniform1i(isVertexLightingUniform, 1);
			glUniform1i(isFragmentLightingUniform, 0);
		}
		else if (gbIsFragment)
		{
			glUniform1i(isFragmentLightingUniform, 1);
			glUniform1i(isVertexLightingUniform, 0);
		}
