...
GLfloat xtrans = -5.0f, ytrans = 2.0f,scalevar=1.0f,y1trans=0.0f;
...
void display(void)
{
	...
	if (ytrans < 0.0)
	{
		glTranslatef(xtrans, y1trans, -10.0f);
	}
	else {
	...
	for (int i = 0.0f; i < numPoints; i++)
	{
		...
		glVertex3f(cos(angle), sin(angle)*scalevar, 0.0f);
		}
	}
	glEnd();
...
void update(void)
{
	xtrans = xtrans + 0.04;
	ytrans = ytrans - 0.01;
	y1trans = y1trans + 0.01;
	if (ytrans < 0.5)
	{
		scalevar = scalevar - 0.01;
