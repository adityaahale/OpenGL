...
void display(void)
{
	...
	glPointSize(2.0f);
	for (float i = 1.0; i < 1.8; i += 0.2)
	{
		for (float j = 1.0; j > 0.4; j -= 0.2)
		{
			glBegin(GL_POINTS);
	...
	for (float i = -1.0; i < 1.0; i += (float)2 / 3)
	{
		glBegin(GL_LINES);
		glVertex2f(-1.0, -i);
		glVertex2f(1.0, -i);
	...
	for (float i = -1.0; i < 1; i += (float)2 / 4)
	{
		glBegin(GL_LINES);
		glVertex2f(1.0, i);
		glVertex2f(-1.0, i);
	...
	glLoadIdentity();
	glTranslatef(3.0f, -4.5f, -15.0f);
	int coun = 0;
	for (float i = 0.0f; i < 3; i++, coun++)
	{
		for (float j = 0.0f; j < 3; j++)
		{
			glBegin(GL_QUADS);
			glColor3f((GLfloat)colourArray[coun][0], (GLfloat)colourArray[coun][1], (GLfloat)colourArray[coun][2]);
			glVertex2f(i, j);
			glVertex2f(i + 1, j);
			glVertex2f(i + 1, j + 1);
			glVertex2f(i, j + 1);
