...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener
{
	...
	private void initialize(GL10 gl){
		...
		//Create a buffer to pass our float data to GPU in native fashion
		byteBuffer =  ByteBuffer.allocateDirect(pyramidColors.length * 4); //4 is size of float
		byteBuffer.order(ByteOrder.nativeOrder()); //Detect native machine endianess and use it
		...
		//Create a buffer to pass our float data to GPU in native fashion
		byteBuffer =  ByteBuffer.allocateDirect(cubeColors.length * 4); //4 is size of float
		byteBuffer.order(ByteOrder.nativeOrder()); //Detect native machine endianess and use it
