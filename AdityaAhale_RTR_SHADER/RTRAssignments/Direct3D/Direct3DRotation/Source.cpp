...
HRESULT initialize(void)
{
	...
	bufferDescColors.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = gpID3D11Device->CreateBuffer(&bufferDescColors, NULL,
		&gpID3D11Buffer_CubeColorBufferAta);
	if (FAILED(hr))
	{
		fopen_s(&gpFileAta, gszLogFileNameAta, "a+");
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_CubeColorBufferAta, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresourceColorsSquare);
	memcpy(mappedSubresourceColorsSquare.pData, squareColorAta, sizeof(squareColorAta));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_CubeColorBufferAta, 0);
...
void display()
{
	...
	//color
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_CubeColorBufferAta, &stride, &offset);
