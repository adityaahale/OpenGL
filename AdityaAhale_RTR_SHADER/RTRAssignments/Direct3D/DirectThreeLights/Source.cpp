...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCodeAta =
		...
		"output.lightTwoDirection=lightTwoPosition.xyz-eyeCoords.xyz;" \
	...
	const char *pixelShaderSourceCodeAta =
		...
		"float3 normalizedlightTwoDirection=normalize(output.lightTwoDirection);" \
		"float tNormDotLightTwoDir = max(dot(normalizedtNorm,normalizedlightTwoDirection),0.0);" \
		"float4 ambientTwo = laTwo * ka;" \
		"float4 diffuseTwo = ldTwo * kd * tNormDotLightTwoDir;" \
		"float3 reflectionVectorTwo = reflect(-normalizedlightTwoDirection,normalizedtNorm);" \
		"float4 specularTwo = lsTwo * ks * pow(max(dot(reflectionVectorTwo,normalizedviewerVector),0.0), materialShininess);" \

		"phong_ads_light=ambientZero + diffuseZero + specularZero + ambientOne + diffuseOne + specularOne + ambientTwo + diffuseTwo + specularTwo;" \
		"}" \
		"\n" \
		"else" \
		"{" \
...
void display()
{
	...
	light0Position[0] = 0.0f;
	light0Position[1] = sinf(angleSquareAta) * myRadius;
	light0Position[2] = cosf(angleSquareAta) * myRadius;

	light1Position[0] = sinf(angleSquareAta) * myRadius;
	light1Position[1] = 0.0f;
	light1Position[2] = cosf(angleSquareAta) * myRadius;

	light2Position[0] = cosf(angleSquareAta) * myRadius;
	light2Position[1] = sinf(angleSquareAta) * myRadius;
	...
	if (gbLight == true)
	{
		...
		constantBuffer.laTwo = XMVectorSet(light2Ambient[0], light2Ambient[1], light2Ambient[2], light2Ambient[3]);
		constantBuffer.ldTwo = XMVectorSet(light2Diffuse[0], light2Diffuse[1], light2Diffuse[2], light2Diffuse[3]);
		constantBuffer.lsTwo = XMVectorSet(light2Specular[0], light2Specular[1], light2Specular[2], light2Specular[3]);
		...
		constantBuffer.lightTwoPosition = XMVectorSet(light2Position[0], light2Position[1], light2Position[2], light2Position[3]);
