...
HRESULT initialize(void)
{
	...
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(pyramidVerticesAta);
	...
	memcpy(mappedSubresource.pData, pyramidVerticesAta, sizeof(pyramidVerticesAta));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_PyramidVertexBufferAta, NULL);

	const float pyramidTexAta[] = {
		0.5, 1.0, // front-top
		0.0, 0.0, // front-left
		1.0, 0.0, // front-right
	...
	bufferDescTexs.ByteWidth = sizeof(float) * ARRAYSIZE(pyramidTexAta);
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescTexs, NULL,
		&gpID3D11Buffer_PyramidTextureBufferAta);
	if (FAILED(hr))
	{
		fopen_s(&gpFileAta, gszLogFileNameAta, "a+");
	...
	D3D11_MAPPED_SUBRESOURCE mappedSubresourceTexturePyramid;
	ZeroMemory(&mappedSubresourceTexturePyramid, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_PyramidTextureBufferAta, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresourceTexturePyramid);
	memcpy(mappedSubresourceTexturePyramid.pData, pyramidTexAta, sizeof(pyramidTexAta));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_PyramidTextureBufferAta, 0);

	//cube drawing
	float cubeVerticesAta[] =
	{
		// SIDE 1 ( TOP )
			// triangle 1
			-1.0f, +1.0f, +1.0f,
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(cubeVerticesAta);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL,
	...
	memcpy(mappedSubresource.pData, cubeVerticesAta, sizeof(cubeVerticesAta));
	...
	ZeroMemory(&bufferDescTexs, sizeof(D3D11_BUFFER_DESC));
	bufferDescTexs.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescTexs.ByteWidth = sizeof(float) * ARRAYSIZE(cubeTexAta);
	bufferDescTexs.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescTexs.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = gpID3D11Device->CreateBuffer(&bufferDescTexs, NULL,
	...
	memcpy(mappedSubresourceTextureCube.pData, cubeTexAta, sizeof(cubeTexAta));
	...
	//pyramid texture 
	hr = LoadD3DTexture(L"Stone.bmp", &gpID3D11ShaderResourceViewTexturePyramidAta);
	if (FAILED(hr))
	{
		fopen_s(&gpFileAta, gszLogFileNameAta, "a+");
		fprintf_s(gpFileAta, "ID3D11Device::Stone::LoadD3DTexture() Failed for texture.\n");
		fclose(gpFileAta);
		return(hr);
	}
	else
	{
		fopen_s(&gpFileAta, gszLogFileNameAta, "a+");
		fprintf_s(gpFileAta, "ID3D11Device::Stone::LoadD3DTexture() Succeeded for texture..\n");
		fclose(gpFileAta);
	}
	...
	hr = gpID3D11Device->CreateSamplerState(&samplerDesc, &gpID3D11SamplerStateTexturePyramidAta);
	if (FAILED(hr))
	{
		fopen_s(&gpFileAta, gszLogFileNameAta, "a+");
		fprintf_s(gpFileAta, "ID3D11Device::Stone::CreateSamplerState() Failed for texture.\n");
		fclose(gpFileAta);
		return(hr);
	}
	else
	{
		fopen_s(&gpFileAta, gszLogFileNameAta, "a+");
		fprintf_s(gpFileAta, "ID3D11Device::Stone::CreateSamplerState() Succeeded for texture..\n");
		fclose(gpFileAta);
	}
	//cube texture
	hr = LoadD3DTexture(L"Vijay_Kundali.bmp", &gpID3D11ShaderResourceViewTextureCubeAta);
	if (FAILED(hr))
	{
		fopen_s(&gpFileAta, gszLogFileNameAta, "a+");
		fprintf_s(gpFileAta, "ID3D11Device::Kundali::LoadD3DTexture() Failed for texture.\n");
		fclose(gpFileAta);
		return(hr);
	}
	else
	{
		fopen_s(&gpFileAta, gszLogFileNameAta, "a+");
		fprintf_s(gpFileAta, "ID3D11Device::Kundali::LoadD3DTexture() Succeeded for texture..\n");
		fclose(gpFileAta);
	}
	...
	{
		fopen_s(&gpFileAta, gszLogFileNameAta, "a+");
		fprintf_s(gpFileAta, "ID3D11Device::Kundali::CreateSamplerState() Failed for texture.\n");
		fclose(gpFileAta);
		return(hr);
	}
	else
	{
		fopen_s(&gpFileAta, gszLogFileNameAta, "a+");
		fprintf_s(gpFileAta, "ID3D11Device::Kundali::CreateSamplerState() Succeeded for texture..\n");
		fclose(gpFileAta);
	}
...
void display()
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_PyramidTextureBufferAta, &stride, &offset);

	//bind texture
	gpID3D11DeviceContext->PSSetShaderResources(0, 1,
		&gpID3D11ShaderResourceViewTexturePyramidAta);
	gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerStateTexturePyramidAta);
	...
	offset = 0;
	//vertex
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_CubeVertexBufferAta, &stride, &offset);
	//texture
	stride = sizeof(float) * 2;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_CubeTextureBufferAta, &stride, &offset);
	//bind texture
	gpID3D11DeviceContext->PSSetShaderResources(0, 1,
	...
	worldMatrixAta = XMMatrixTranslation(1.5f, 0.0f, 6.0f);
	worldMatrixAta = XMMatrixScaling(0.75f, 0.75f, 0.75f) * worldMatrixAta;
	worldMatrixAta = XMMatrixRotationX(XMConvertToRadians(angleSquareAta)) * worldMatrixAta;
	worldMatrixAta = XMMatrixRotationY(XMConvertToRadians(angleSquareAta)) * worldMatrixAta;
	worldMatrixAta = XMMatrixRotationZ(XMConvertToRadians(angleSquareAta)) * worldMatrixAta;
