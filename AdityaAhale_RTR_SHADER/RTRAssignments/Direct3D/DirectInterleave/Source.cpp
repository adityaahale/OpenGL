...
HRESULT initialize(void)
{
	...
	const char *pixelShaderSourceCodeAta =
		...
		"phong_ads_light=output.color * myTexture2D.Sample(mySamplerState, output.texcoord) * (ambient + diffuse + specular);" \
		"}" \
		"\n" \
		"else" \
		"{" \
		"phong_ads_light = output.color * myTexture2D.Sample(mySamplerState, output.texcoord);" \
		"}" \
	...
	hr = gpID3D11Device->CreateInputLayout(inputElementDesc, 4, pID3DBlob_vertexShaderCode->GetBufferPointer(),
	...
	//cube texture
	hr = LoadD3DTexture(L"marble.bmp", &gpID3D11ShaderResourceViewTextureCubeAta);
	if (FAILED(hr))
	{
		fopen_s(&gpFileAta, gszLogFileNameAta, "a+");
		fprintf_s(gpFileAta, "ID3D11Device::marble::LoadD3DTexture() Failed for texture.\n");
		fclose(gpFileAta);
		return(hr);
	}
	else
	{
		fopen_s(&gpFileAta, gszLogFileNameAta, "a+");
		fprintf_s(gpFileAta, "ID3D11Device::marble::LoadD3DTexture() Succeeded for texture..\n");
		fclose(gpFileAta);
	}
...
void display()
{
	...
	//vertex
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_PyramidVertexBufferAta, &stride, &offsetPos);
	//color
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_PyramidVertexBufferAta, &stride, &offsetColor);
	gpID3D11DeviceContext->IASetVertexBuffers(2, 1, &gpID3D11Buffer_PyramidVertexBufferAta, &stride, &offsetNormal);
	gpID3D11DeviceContext->IASetVertexBuffers(3, 1, &gpID3D11Buffer_PyramidVertexBufferAta, &stride, &offsetTex);
	...
	worldMatrixAta = scaleMatrix * rotationMatrix * translationMatrix;
	XMMATRIX wvMatrixAta = worldMatrixAta * viewMatrixAta * PerspectiveProjectionMatrixAta;
