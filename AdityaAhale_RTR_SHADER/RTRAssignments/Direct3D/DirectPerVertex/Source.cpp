...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCodeAta =
		...
		"float3 tNorm = normalize(mul((float3x3)mul(viewMatrix, worldMatrix),(float3)normal));" \
		"float3 lightDirection=(float3)normalize(lightPosition-eyeCoords);" \
		"float tNormDotLightDir = max(dot(tNorm,lightDirection),0.0);" \
		"float4 ambient = la * ka;" \
		"float4 diffuse = ld * kd * tNormDotLightDir;" \
		"float3 reflectionVector = reflect(-lightDirection,tNorm);" \
		"float3 viewerVector = normalize(-eyeCoords.xyz);" \
