...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCodeAta =
		...
		"output.tNorm = normalize(mul((float3x3)mul(viewMatrix, worldMatrix),(float3)normal));" \
		"output.lightDirection=normalize(lightPosition.xyz-eyeCoords.xyz);" \
		"output.viewerVector = normalize(-eyeCoords.xyz);" \
		"if(isPerVertex == 1)" \
		"{" \
		"float4 ambient = la * ka;" \
		"float4 diffuse = ld * kd * max(dot(output.lightDirection,output.tNorm),0.0);" \
		"float3 reflectionVector = reflect(-output.lightDirection,output.tNorm);" \
		"float4 specular = ls * ks * pow(max(dot(reflectionVector,output.viewerVector),0.0), materialShininess);" \
	...
	const char *pixelShaderSourceCodeAta =
		...
		"{" \
			"float4 phong_ads_light_out;" \
				"if(isPerVertex == 0)" \
				"{" \
				"float tNormDotLightDir = max(dot(output.tNorm,output.lightDirection),0.0);" \
				"float4 ambient = la * ka;" \
				"float4 diffuse = ld * kd * tNormDotLightDir;" \
				"float3 reflectionVector = reflect(-output.lightDirection,output.tNorm);" \
				"float4 specular = ls * ks * pow(max(dot(reflectionVector,output.viewerVector),0.0), materialShininess);" \
				"phong_ads_light_out = ambient + diffuse + specular;" \
				"}" \
			"else" \
			"{" \
			"phong_ads_light_out = output.phong_ads_light;" \
			"}" \
			"return(phong_ads_light_out);"\
		"}";
